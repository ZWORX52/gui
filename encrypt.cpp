...
void WordleEncryption::Encrypt(char in[], char out[]) {
        ...
        while (in[i]) {
                ...
                out[i] = static_cast<char>((loopedchr + in[i] + i - 97) % 26 + 97);
                i++;
        }
}
...
void WordleEncryption::Decrypt(char in[], char out[]) {
        ...
        while (in[i]) {
                ...
                out[i] = static_cast<char>((tmpresult) % 26 + 97);
                i++;
        }
}
...
void WordleEncryption::UpdateWindow(bool *open) {
        ...
        if (ImGui::BeginMenuBar()) {
                if (ImGui::BeginMenu("Copy")) {
                        if (ImGui::MenuItem("Encrypted -> Decrypt")) {
                                ...
                                snprintf(WordleDecryptionIn, BUFFER_SIZE, "%s", WordleEncryptionOut);
                        }
                        if (ImGui::MenuItem("Decrypted -> Encrypt")) {
                                Utils::ClearBuffer(WordleEncryptionIn);
                                snprintf(WordleEncryptionIn, BUFFER_SIZE, "%s", WordleDecryptionOut);
                        }
                        ImGui::EndMenu();
                }
                ImGui::EndMenuBar();
        ...
        ImGui::InputText("Encryption input", WordleEncryptionIn, BUFFER_SIZE,
                        ImGuiInputTextFlags_CallbackCharFilter, EncryptFilter);
        ...
        ImGui::InputText("Decryption input", WordleDecryptionIn, BUFFER_SIZE,
                        ImGuiInputTextFlags_CallbackCharFilter, EncryptFilter);
